    ...
    try:
        if data.startswith("CL-"):
            ...
            if not token or len(token) > 32:
                logger.error("CHANNEL_LINK_INVALID_TOKEN user_id=%s token=%s", message.from_user.id, token)
                await message.reply_text("This file link is invalid or expired.")
                return
            link_data = await db.get_channel_link(token)
            if not link_data:
                logger.error("CHANNEL_LINK_TOKEN_NOT_FOUND user_id=%s token=%s", message.from_user.id, token)
                await message.reply_text("This file link is invalid or expired.")
                return
            chat_id = link_data["chat_id"]
            message_ids = link_data["message_ids"]
            logger.info("CHANNEL_LINK_TOKEN_RESOLVED user_id=%s token=%s kind=%s", message.from_user.id, token, link_data["kind"])
        else:
            parts = data.split("|")
            kind = parts[0]
            if kind == "CF" and len(parts) == 3:
                ...
                if last_id - first_id > 200:
                    ...
                    return
                message_ids = [message_id for message_id in range(first_id, last_id + 1)]
            else:
                # Support links generated by the previous release, including
                # negative channel IDs separated with hyphens.
                legacy_parts = data.split("-")
                if legacy_parts[0] == "CF" and len(legacy_parts) == 3:
                    chat_id, message_id = int(legacy_parts[1]), int(legacy_parts[2])
                    message_ids = [message_id]
                elif legacy_parts[0] == "CB" and len(legacy_parts) == 5 and not legacy_parts[1]:
                    chat_id, first_id, last_id = map(int, legacy_parts[2:])
                    if last_id - first_id > 200:
                        await message.reply_text("This batch is too large. Please keep a batch within 200 messages.")
                        return
                    message_ids = [message_id for message_id in range(first_id, last_id + 1)]
...
async def start(client, message):
    ...
    fileid = None
    if not data.startswith(("file_", "filep_", "files_", "allfiles", "sendfiles", "BATCH-", "DSTORE-", "CF-", "CB-", "CF|", "CB|", "CL-", "verify-")):
        try:
            decoded_data = base64.urlsafe_b64decode(data + "=" * (-len(data) % 4)).decode("ascii")
            if decoded_data.startswith(("file_", "filep_", "CF-", "CB-", "CF|", "CB|")):
                data = decoded_data
    ...
    if (
        ...
        and not data.startswith(("file_", "filep_", "files_", "allfiles", "sendfiles", "BATCH-", "DSTORE-", "CF-", "CB-", "CF|", "CB|", "CL-"))
    ):
